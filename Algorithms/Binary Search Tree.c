...
int deleteKey(n** root, int key)
{
    ...
    if(!cur->left && !cur->right)    // Deletion of a leaf node
    {
        if(cur == *root)
            *root = NULL;
    ...
    else if((cur->left && !cur->right) || (cur->right && !cur->left))   // Deletion of Half Node
    {
        if(cur == *root)
        {
            if(cur->left)
    ...
    else if(cur->left && cur->right)      // Deletion of Full Node
    {
        n*ssr = cur->right;
        n*par_ssr = NULL;
    ...
    while(1)
    {
        ...
        switch(ch)
        {
            ...
            case 15:
                ...
                break;
            case 16:
                printf("Level with the highest suma and its index\n");
            ...
            case 38:
                ...
                break;
            case 39:
                printf("Level number of level with highes number of nodes = %d\n",getLevelWithMaxNodes(root));
                break;
            case 40:
