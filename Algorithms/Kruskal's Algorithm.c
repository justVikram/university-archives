...
static void applyKruskalAlgorithm (EDGE E [], EDGE SelectedEdges [], int V)
{
    for (int i = 0; i < NumberOfEdgesStored; i++)
    {
        int SourceVertex = E [i].SourceVertex;
        int AdjacentVertex = E [i].AdjacentVertex;
        ...
        {
            SelectedEdges [NumberOfEdgesSelected] = E [i];
...
void kruskalAlgorithm (GRAPH G [], int V)
{
    ...
    storeAllEdges (E, G, V);
    sortEdgesInAscOrder (E, NumberOfEdgesStored, sizeof (EDGE), qSortComparator);

    applyKruskalAlgorithm (E, SelectedEdges, V);
