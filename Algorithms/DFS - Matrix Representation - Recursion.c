...
void _depthFirstSearch (int Graph [][10], int V, int SomeVertex, int IsVisited [])
{
    printf ("%d\t", SomeVertex);
    ...
    for (int i = 0; i < V; i++)
    {
        int IsAdjacent = Graph [SomeVertex][i] ;
        
        if (IsAdjacent && ! IsVisited [i])
            _depthFirstSearch(Graph, V, i, IsVisited);
    }
}
